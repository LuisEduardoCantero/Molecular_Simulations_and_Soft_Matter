...
!!
	...
	rmin2=diam/dd+H-1.2840d-1*dlt
	rmin3=diam/dd+H+(1.d0-1.2840d-1)*dlt
	rmin4=diam/dd+H+H2-d0
	rmax=dsqrt(100.d0/dk)+diam/dd+H+H2
        ad=diam			!! Adimensionalización
	escR=dd/ad
	Write(*,*)escR*rmin,diam/dd
	Write(*,*)escR*rmin2,escR*rmin3,escR*rmin4,rmax
!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!
!!	Escalamiento
...
open(15,file='conf_fin.dat', status='unknown')
...
!			write(*,*)nprom
		...
		call newconfig(x,y,z)
	enddo
	open(1,file='conf_fin1.dat',status='unknown')
	Do i=1,np
	...
	write(15,*)'excess energy/kTN=',eex
	write(15,*)'sigma(excess energy/kTN)=',sige
	do i=1,nr
		r(i)=dr/2.d0+dr*(i-1)
...
!!		Escalamiento
	...
	enddo
	open(16,file='Fself1.dat',status='unknown')
	qqs=2.0d0*pi*EscR
...
!! 	Escalamiento
		t1(i+1)=EscD*t(i+1) 
	write(16,*)EscD*t(i+1),Fs/(Np*(nprom-i)*(3.d0*(NSS**3-1.d0))),Fs2/(Np*(nprom-i))       
